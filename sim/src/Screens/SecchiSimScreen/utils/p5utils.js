...
export const CONSTANTS = {
  ...
  export const drawWater = (p5, width, height, currentDepth, maxDepth) => {
    for (let y = 0; y < height; y++) {
      ...
      const waterColor = p5.color(0, 50, 100, alpha);
  ...
  export const drawSecchiDisk = (p5, x, y, visibility) => {
    p5.push();
    p5.translate(x, y);
    
    // Draw disk with segments
    const radius = CONSTANTS.DISK_DIAMETER / 2;
    p5.fill(255 * visibility); // White color that fades with visibility
    p5.stroke(0);
    p5.strokeWeight(2);
    
    p5.beginShape();
    for (let i = 0; i < CONSTANTS.DISK_SEGMENTS * 2; i++) {
      const angle = (i * Math.PI) / CONSTANTS.DISK_SEGMENTS;
      if (i % 2 === 0) {
        p5.fill(255 * visibility);
      } else {
        p5.fill(0);
      }
      p5.arc(0, 0, radius * 2, radius * 2, angle, angle + Math.PI / CONSTANTS.DISK_SEGMENTS);
    }
    p5.endShape();
