...
const Productive = forwardRef(({ settings, onSettingChange }, ref) => {
    ...
    const loadUserAttempts = async () => {
      ...
      try {
        ...
        if (quizDoc.exists()) {
          ...
          const remainingRetries = data.Productive_RetryCount ?? 3;
          setRetryCount(remainingRetries);
          
          // Check if module is disabled
          const isDisabled = data.Productive_Disabled ?? false;
  ...
  useEffect(() => {
    sketchRef.current = new p5((p) => {
      ...
      p.preload = () => {
        p.waterImage = p.loadImage('/productiveLake.png');
      };
  ...
  const handleAttemptSubmit = async () => {
    ...
    try {
      ...
      await runTransaction(db, async (transaction) => {
        ...
        if (!quizDoc.exists()) {
          ...
          transaction.set(quizDocRef, {
            Productive_RetryCount: 3,
            Productive_Disabled: false,
        ...
        const attemptKeys = Object.keys(scoresData)
          .filter(key => key.startsWith('Productive_Attempt_'))
          .sort((a, b) => {
        ...
        const quizUpdates = {
          Productive_RetryCount: newRetryCount,
          Productive_Disabled: shouldDisableModule,
          lastUpdated: new Date().toISOString()
        };

        // Handle target depth based on pass/fail
        if (isPassed) {
          quizUpdates.Productive_TargetDepth = deleteField();
        } else {
          quizUpdates.Productive_TargetDepth = targetDepth;
        }
        ...
        transaction.set(scoresDocRef, {
          ...scoresData,
          [`Productive_Attempt_${nextAttemptNumber}`]: attemptData,
          lastUpdated: new Date().toISOString()
        }, { merge: true });
      });
      ...
      if (isPassed) {
        setModuleDisabled(true);
        setPopupMessage(`Congratulations! You've successfully completed the Productive Lake module with a depth of ${currentDepth}m! You may attempt this module again next year to maintain your certification.`);
        setShowPopup(true);
