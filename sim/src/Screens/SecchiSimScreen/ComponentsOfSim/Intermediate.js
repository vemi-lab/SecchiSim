...
const Intermediate = forwardRef(({ settings, onSettingChange }, ref) => {
  ...
  const lakeConfig = LAKE_CONFIGS.INTERMEDIATE;
    ...
    const loadUserAttempts = async () => {
      ...
      try {
        ...
        if (quizDoc.exists()) {
          ...
          const remainingRetries = data.Intermediate_RetryCount ?? 3;
          setRetryCount(remainingRetries);
          
          // Check if module is disabled
          const isDisabled = data.Intermediate_Disabled ?? false;
  ...
  const handleAttemptSubmit = async () => {
    ...
    try {
      ...
      await runTransaction(db, async (transaction) => {
        ...
        if (!quizDoc.exists()) {
          ...
          transaction.set(quizDocRef, {
            Intermediate_RetryCount: 3,
            Intermediate_Disabled: false,
        ...
        const attemptKeys = Object.keys(scoresData)
          .filter(key => key.startsWith('Intermediate_Attempt_'))
          .sort((a, b) => {
        ...
        const quizUpdates = {
          Intermediate_RetryCount: newRetryCount,
          Intermediate_Disabled: shouldDisableModule,
          lastUpdated: new Date().toISOString()
        };

        // Handle target depth based on pass/fail
        if (isPassed) {
          quizUpdates.Intermediate_TargetDepth = deleteField();
        } else {
          quizUpdates.Intermediate_TargetDepth = targetDepth;
        }
        ...
        transaction.set(scoresDocRef, {
          ...scoresData,
          [`Intermediate_Attempt_${nextAttemptNumber}`]: attemptData,
          lastUpdated: new Date().toISOString()
        }, { merge: true });
      });
      ...
      if (isPassed) {
        setModuleDisabled(true);
        setPopupMessage(`Congratulations! You've successfully completed the Intermediate Lake module with a depth of ${currentDepth}m! You may attempt this module again next year to maintain your certification.`);
        setShowPopup(true);
