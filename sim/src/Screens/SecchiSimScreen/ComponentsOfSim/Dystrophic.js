...
const Dystrophic = forwardRef(({ settings, onSettingChange }, ref) => {
  ...
  const lakeConfig = LAKE_CONFIGS.DYSTROPHIC;
    ...
    const loadUserAttempts = async () => {
      ...
      try {
        ...
        if (quizDoc.exists()) {
          ...
          const remainingRetries = data.Dystrophic_RetryCount ?? 3;
          setRetryCount(remainingRetries);
          
          // Check if module is disabled
          const isDisabled = data.Dystrophic_Disabled ?? false;
  ...
  const handleAttemptSubmit = async () => {
    ...
    try {
      ...
      await runTransaction(db, async (transaction) => {
        ...
        if (!quizDoc.exists()) {
          ...
          transaction.set(quizDocRef, {
            Dystrophic_RetryCount: 3,
            Dystrophic_Disabled: false,
        ...
        const attemptKeys = Object.keys(scoresData)
          .filter(key => key.startsWith('Dystrophic_Attempt_'))
          .sort((a, b) => {
        ...
        const quizUpdates = {
          Dystrophic_RetryCount: newRetryCount,
          Dystrophic_Disabled: shouldDisableModule,
          lastUpdated: new Date().toISOString()
        };

        // Handle target depth based on pass/fail
        if (isPassed) {
          quizUpdates.Dystrophic_TargetDepth = deleteField();
        } else {
          quizUpdates.Dystrophic_TargetDepth = targetDepth;
        }
        ...
        transaction.set(scoresDocRef, {
          ...scoresData,
          [`Dystrophic_Attempt_${nextAttemptNumber}`]: attemptData,
          lastUpdated: new Date().toISOString()
        }, { merge: true });
      });
      ...
      if (isPassed) {
        setModuleDisabled(true);
        setPopupMessage(`Congratulations! You've successfully completed the Dystrophic Lake module with a depth of ${currentDepth}m! You may attempt this module again next year to maintain your certification.`);
        setShowPopup(true);
